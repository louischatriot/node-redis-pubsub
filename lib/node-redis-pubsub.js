...
function NodeRedisPubsub(options){
  ...
  options || (options = {});
  let port = options && options.port || 6379;   // 6379 is Redis' default
  let host = options && options.host || '127.0.0.1';
  let auth = options && options.auth
...
NodeRedisPubsub.prototype.on = NodeRedisPubsub.prototype.subscribe = function(channel, handler, callback){
  ...
  this.receiver.on('pmessage', (pattern, _channel, message)=>{
    if(this.prefix + channel === pattern){ handler(JSON.parse(message), _channel); }
  });
